...
class LearningSwitch (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    if packet.type == packet.IP_TYPE:
      #Taking some stuff fomr dhcpd pox, stripped down
      ipp = packet.find('ipv4')
      nwp = ipp.payload
      print "\n\n"
      if packet.find("icmp"):
        print "Here's a PING!"
        print "From: " + str(ipp.srcip) + " to: " + str(ipp.dstip)
        print "From: " + str(packet.src) + " to: " + str(packet.dst)
        print "Coming in from " + str(self.macToPort[packet.src]) + " and going out to " + str(self.macToPort[packet.dst])
        pass
      elif nwp.srcport == pkt.dhcp.CLIENT_PORT or nwp.srcport == pkt.dhcp.SERVER_PORT:
        bootPk = nwp.payload
        dhcpOpt = bootPk.options.get(bootPk.MSG_TYPE_OPT)
        sId = bootPk.options.get(bootPk.SERVER_ID_OPT)
      ...
      print "\n\nHERE's the arp! Headed to: " + str(packet.payload.protodst)
      print "From: " + str(packet.payload.protosrc)
      inPort = ''
      outPort = ''
      if packet.src not in  self.macToPort:
        inPort = "Broadcst"
      else:
        inPort = self.macToPort[packet.src]
    ...
    if packet.dst.is_multicast:
      ...
      if ipp and ipp.dstip in self.ipToPort:
        ...
        print "installing flow for %s.%i -> %s.%i" % (packet.src, event.port, packet.dst, port)
